...
<a-button @click="vip">vip</a-button>
...
export default {
    ...
    methods: {
        ...
        vip(){
            this.onchange()
            //this.getvip(this.userId)
            //
            if (this.vipp==1){
                this.finalPrice=this.finalPrice*0.9
            alert("你是VIP用户，已经为您9折优惠")
            }
            else{
                alert("你还不是是vip用户，消费满10000元，享受VIP福利")
            }
        ...
        onchange() {
            ...
            if(this.checkedList.length>0){
                ...
                this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id)!=-1).forEach(item => this.finalPrice= this.finalPrice*(1-item.discount))
            }else{
